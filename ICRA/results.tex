...
\begin{enumerate}
  ...
  \item \textbf{GaitNet-Single:} GaitNet with the contact-count
    constraint tightened so only one leg may swing at a time.
...
\begin{figure}[t]
  ...
  \caption{Unitree Go1 hardware test track. Terrain difficulty is the
    percentage of the track area that cannot be stepped on.
    \textit{Placeholder: to be replaced with a hardware-trial photo.}}
...
Before the hardware system in \autoref{sec:methodology}, our
preliminary framework \cite{Sullivan2025} evaluated cost-ablated
...
no-action option. They did not use \changed{the exhaustive
...
\changed{Deployment-time enumeration changes which candidates are
