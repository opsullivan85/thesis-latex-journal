...
coordination anywhere in its architecture. At each 40\,ms control
tick, GaitNet commits at most one footstep, drawn from a small set of
terrain-filtered candidates and locally refined against its own
...
itself (GaitNet-Single, GaitNet-Trot), achieving \textbf{XX\%} versus
\textbf{XX\%} mean success rate across matched terrain difficulties,
with the gap widening at the highest difficulties tested. Simulation
...
GaitNet's concurrency is capped at two simultaneous swing legs by an
explicit feasibility constraint requiring at least three legs in
ground contact; this was a deliberate design choice for stability
margin, but its effect on performance at the extremes of terrain
difficulty is untested and worth ablating directly. The policy is
also trained on a candidate distribution, sixteen randomly sampled
positions per leg, that differs from the distribution it is deployed
against, eight candidates locally refined by gradient ascent; we
treat this as intentional but have not yet characterized how much
the refinement step contributes beyond what random sampling alone
would achieve. Both simulation and hardware deployments depend on an
...
quantified. Finally, candidate refinement operates on a discretized
terrain mask rather than a continuous representation of the
workspace, so the gains attributable to gradient-based refinement
specifically, as opposed to the terrain mask and re-sampling alone,
