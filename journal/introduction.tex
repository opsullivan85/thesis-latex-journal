...
\changed{Other planners remove the periodic schedule without using a
...
\changed{End-to-end policies put the same discrete choices inside one
...
shi_terrain-aware_2023, xie_glide_2023}. \changed{Another design
...
configured gait sets the swing timing. \changed{GaitNet differs in
...
\begin{enumerate}
  ...
  \item \changed{A per-candidate formulation in which the learned
...
This paper extends our preliminary framework \cite{Sullivan2025}. The
earlier work introduced the single-footstep, single-tick action space.
It reported simulation results with randomly sampled candidate
footholds. \changed{Here, deployment replaces random sampling with
