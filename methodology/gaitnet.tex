...
The tracking state contains the current foot positions
$\mathbf p_{f,xy}^{b} \in \mathbb{R}^{8}$ in the base frame,
base height $p_{b,z}$ in the world frame, body linear
$\mathbf v_b \in \mathbb{R}^{3}$ and angular
$\mathbf \omega_b \in \mathbb{R}^{3}$ velocities, and commanded
velocity $\mathbf v^{usr} = [v_x, v_y, \omega_z]$.
The remaining terms are the per-foot contact state
$\mathbf c \in \{0,1\}^{4}$ and the projected gravity vector
$\mathbf g_b \in \mathbb{R}^{3}$. Using $\mathbf{x_g}$ and
$\mathbf{f_c}^{\prime}$,
